...
const checkSettingsAnimalProfileForm = () => {
	let photo= $("#settings-animal-profile-photo").val();
	...
	query({
		type:'update_animal',
		params:[photo,name,type,breed,description,animalId]
	}).then(d=>{
		if(d.error) throw d.error;
		window.history.back();
	})
...
const checkSettingsProfilePasswordPage = async() => {

	let oldpassword = $("#settings-change-password-1").val();
	
	let newpassword = $("#settings-change-password-2").val();
	let newpassword2 = $("#settings-change-password-3").val();
	...
	if(newpassword!=newpassword2) {
		...
		query({
			...
			params:[newpassword,userId,oldpassword]
		}).then(d=>{
			console.log(d);
			if(d.error) throw d.error;
			window.history.back();
