...
const AnimalProfilePage = async() => {
	...
	makeMarkers2(map_el,locations.result);

	map_el.data("markers").forEach((o,i)=>{
		o.addListener("click",function(){
			// INFOWINDOW EXAMPLE
			map_el.data("infoWindow").open(map_el.data("map"),o);
			map_el.data("infoWindow").setContent(makeRecentProfile2(locations.result[i]))
...
const AddLocationPage = async() => {
	...
	map_el.data("map").addListener("click",function(e) {
		...
		makeMarkers(map_el,[{lat:e.latLng.lat(),lng:e.latLng.lng(),icon:'images/icon/pin.png'}])
	})
}
