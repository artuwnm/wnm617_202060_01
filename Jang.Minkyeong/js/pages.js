...
const ProfilePage = async() => {
	...
	let animals = await query({
		type:"animals_by_user_id",params:[sessionStorage.userId]});
	let locations = await query({
		type:"locations_by_user_id",params:[sessionStorage.userId]});
...
const AddLocationPage = async() => {
	...
	map_el.data("map").addListener("click",function(e) {
		...
		makeMarkers(map_el,[{lat:e.latLng.lat(),lng:e.latLng.lng(),icon:'img/spot.png'}])
	})
}
...
const ChooseAnimalPage = async () => {
	...
	$("#add-location-animal-id")
	.html(makeSelectOptions(animals.result.map(o=>([o.id,o.breed]))));
}
